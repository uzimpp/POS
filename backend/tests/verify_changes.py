...
def test_verification():
    ...
    try:
        ...
        # Create Branch
        branch_data = BranchCreate(name="Main Branch", address="123 Main St", phone="555-0100")
        ...
        stock = Stock(branch_id=branch.branch_id, stk_name="Coffee Beans", amount_remaining=10.0, unit="kg")
        db.add(stock)
        db.commit()
        db.refresh(stock)
        ...
        deleted_stock = db.query(Stock).filter(Stock.stock_id == stock.stock_id).first()
        ...
        try:
            ...
            if e.status_code == 400 and "inactive branch" in e.detail:
                print("   [PASS] Correctly blocked creating employee in inactive branch.")
            else:
                print(f"   [FAIL] Raised wrong exception: {e}")

        print("5. Verifying Multi-Branch Filter...")
        # Create another branch and employee
        branch2_data = BranchCreate(name="Second Branch", address="456 Elm St", phone="555-0200")
        branch2 = branches_router.create_branch(branch2_data, db)
        emp2_data = EmployeeCreate(branch_id=branch2.branch_id, role_id=role.role_id, first_name="Jane", last_name="Doe", salary=60000, is_active=True)
        emp2 = employees_router.create_employee(emp2_data, db)
        
        # Test Filter
        # 1. Filter by Branch 1 (which is deleted/inactive but still exists in DB)
        res1 = employees_router.get_employees(branch_ids=[branch.branch_id], db=db)
        print(f"   Filter [Branch 1]: Found {len(res1)} employees. (Expect 1)")
        assert len(res1) == 1
        assert res1[0].branch_id == branch.branch_id
        
        # 2. Filter by Branch 2
        res2 = employees_router.get_employees(branch_ids=[branch2.branch_id], db=db)
        print(f"   Filter [Branch 2]: Found {len(res2)} employees. (Expect 1)")
        assert len(res2) == 1
        
        # 3. Filter by Both
        res3 = employees_router.get_employees(branch_ids=[branch.branch_id, branch2.branch_id], db=db)
